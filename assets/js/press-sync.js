window.PressSync = ( function( window, document, $ ) {
    ...
    app.updateProgressBar = function( objects_to_sync, total_objects_processed, total_objects, request_time ) {
        const progress_complete = (total_objects_processed / total_objects) * 100;
        let percent_complete = Math.floor(progress_complete);
        ...
        let progress_string = total_objects_processed + '/' + total_objects + ' ' + objects_to_sync + ' synced';

        if ( request_time ) {
            // Estimate time remaining.
            let remaining_time = (((total_objects - total_objects_processed) / app.PAGE_SIZE) * request_time);
            remaining_time = remaining_time / 60 / 60;
            let time_left_suffix = 'hours';
     ...
     */
    app.syncData = function( paged, objects_to_sync, next_args = null ) {
        ...
        $.ajax({
            ...
            const request_time = (new Date().getTime() - start_time) / 1000;
