...
class Press_Sync {
	...
	public function init_connection( $remote_domain = '' ) {
		...
		$this->remote_domain = ( $remote_domain ) ? trailingslashit( $remote_domain ) : untrailingslashit(
			get_option( 'ps_remote_domain' )
		);
	}
...
SQL;
	...
	public function get_featured_image( $post_id ) {
		...
		// @TODO Filterable?
		$media_urls = array(
			'local_media' => home_url( 'wp-content/uploads/' . get_post_meta( $thumbnail_id,
					'_wp_attached_file',
					true ) ),
	...
	public function sync_batch( $content_type = 'post', $settings = array(), $next_page = 1 ) {
		...
		$objects       = $settings['local_folder'] ? $this->get_local_objects_to_sync(
			$settings['local_folder'],
			$content_type
		) : $this->get_objects_to_sync( $content_type, $next_page, $taxonomies );
		...
		return array(
			...
			'total_objects_processed' => ( $next_page * $this->settings['ps_page_size'] ) - ( $this->settings['ps_page_size'] - count(
						$objects
					) ),
